...
Я не смог (по крайней мере пока) объяснить это иначе чем тем, что раньше было как-то по одному, а теперь как-то по
другому

### Лимиты

- user_engine_max_messages_cnt = 15M
    - если у пользователя сообщений больше 15M, он считается overflow
- user_engine_cut_messages_left = 10M
    - при резке остаётся 10M сообщений
- доступный хвост исторических событий = 15K
- доступно истории через обычный longpoll = 256
- MAX_USER_FOLDERS = 30
- MAX_FOLDER_EXPLICIT_PEERS = 500
- USER_ENGINE_MAX_PEERS_CNT = 150K
    - если у мользователя чатов больше 150к и включён флаг drop_old_peers, то
- USER_ENGINE_CUT_PEERS_LEFT = 100K
    - режем до 100к, дропаем самые старые
- max_messages_used_to_calculate_folder_unread_counts = 2000
    - считать сообщения в папках дорого